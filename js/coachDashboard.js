...
(function () {
  ...
  function ensureRoot() {
    ...
    el.innerHTML =
      ...
      '    </div>' +
      '    <button id="cdRefresh" style="border:0;background:transparent;cursor:pointer;font-size:13px;color:var(--ink3,#9a9da3);padding:6px;">Refresh</button>' +
      '  </header>' +
      '  <input id="cdSearch" type="search" placeholder="Search athletes by name" ' +
      '     style="width:100%;box-sizing:border-box;padding:10px 12px;border:1px solid var(--line,#ebebe8);border-radius:12px;margin-bottom:12px;font-size:14px;" />' +
  ...
  var STATUS_META = {
    needs_attention: { label: "Needs attention", order: 0, color: "var(--bad,#c0272d)" },
    monitor:         { label: "Monitor",         order: 1, color: "var(--warn,#c98a1e)" },
    no_recent_data:  { label: "No recent data",  order: 2, color: "var(--ink3,#9a9da3)" },
    on_track:        { label: "On track",         order: 3, color: "var(--good,#1f9d5b)" }
  };
  ...
  function renderRoster() {
    ...
    if (state.loading) { body.innerHTML = '<div style="padding:32px;text-align:center;color:var(--ink3,#9a9da3);">Loading roster…</div>'; return; }
    if (state.error) {
      body.innerHTML = '<div style="padding:24px;text-align:center;color:var(--bad,#c0272d);">' +
        esc(state.error) + '<br><button id="cdRetry" style="margin-top:10px;padding:8px 14px;border-radius:10px;border:1px solid var(--line,#ebebe8);background:transparent;cursor:pointer;">Try again</button></div>';
      var rb = document.getElementById("cdRetry"); if (rb) rb.addEventListener("click", function () { loadAndRender(true); });
      return;
    }
    if (!state.athletes.length) {
      body.innerHTML = '<div style="padding:40px 16px;text-align:center;color:var(--ink3,#9a9da3);">' +
    ...
    if (!visible.length) { body.innerHTML = '<div style="padding:32px;text-align:center;color:var(--ink3,#9a9da3);">No athletes match “' + esc(q) + '”.</div>'; return; }
    ...
    body.innerHTML = visible.map(function (a) {
      ...
      return '' +
        '<button class="cd-card" data-athlete="' + esc(a.athlete_id) + '" style="width:100%;text-align:left;border:1px solid var(--line,#ebebe8);background:var(--card,#f6f6f4);border-radius:14px;padding:12px 14px;margin-bottom:10px;cursor:pointer;display:block;">' +
        ...
        '        <span style="flex:0 0 auto;font-size:11px;font-weight:600;color:var(--paper,#ffffff);background:' + meta.color + ';border-radius:var(--r-pill);padding:2px 8px;">' + esc(meta.label) + '</span>' +
        '      </div>' +
        '      <div style="font-size:12px;color:var(--ink3,#9a9da3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px;">' +
                esc(sport) + ' · ' + fmtVal(a.goal) + '</div>' +
        (reasons ? '      <div style="font-size:12px;color:' + meta.color + ';margin-top:3px;">' + reasons + '</div>' : '') +
        '    </div>' +
        '  </div>' +
        '  <div style="display:flex;gap:14px;margin-top:10px;font-size:12px;color:var(--ink2,#6d7075);flex-wrap:wrap;">' +
  ...
  async function openAthlete(athleteId) {
    ...
    drawer.hidden = false;
    drawer.innerHTML = '<div style="padding:24px;text-align:center;color:var(--ink3,#9a9da3);">Loading athlete…</div>';
    var res = await api("athlete", { query: { athlete_id: athleteId } });
    if (!res.ok || !res.body || !res.body.athlete) {
      drawer.innerHTML = '<div style="padding:24px;text-align:center;color:var(--bad,#c0272d);">' +
        (res.status === 403 ? "You are not assigned to this athlete." : "Could not load this athlete.") +
        '<br><button id="cdDrawerClose" style="margin-top:10px;padding:8px 14px;border-radius:10px;border:1px solid var(--line,#ebebe8);background:transparent;cursor:pointer;">Close</button></div>';
  ...
  function renderDrawer(ath, rosterEntry) {
    ...
    }).join("") || '<li style="color:var(--ink3,#9a9da3);">No recent data</li>';
    var wk = ath.week_planned_vs_completed || {};
    var pv = function (v, s) { return v == null ? "—" : esc(v) + (s || ""); };

    drawer.hidden = false;
    drawer.innerHTML =
      '<div style="position:fixed;inset:0;background:var(--backdrop,rgba(20,20,22,.45));z-index:60;" id="cdOverlay"></div>' +
      ...
      '      <div style="font-size:12px;color:var(--ink3,#9a9da3);">' + esc((SPORT_LABEL[ath.primary_sport] || "—")) + (ath.goal ? ' · ' + esc(ath.goal) : '') + '</div></div>' +
      '    </div>' +
      ...
      (reasons ? '<div style="margin-bottom:14px;"><div style="font-weight:600;font-size:13px;margin-bottom:6px;">Needs attention</div><ul style="margin:0;padding-left:18px;font-size:13px;">' + reasons + '</ul>' +
        '<button id="cdReview" style="margin-top:8px;padding:7px 12px;border-radius:10px;border:1px solid var(--line,#ebebe8);background:transparent;cursor:pointer;font-size:13px;">Mark reviewed</button></div>' : '') +
