/*
 * ══════════════════════════════════════════════════════════════════════
 *  Athlevo — Aerobic Pace Calibration  (safer Recovery / Easy / Long Run)
 * ══════════════════════════════════════════════════════════════════════
 *
 *  Fixes the "easy/recovery pace is too fast" problem. Today every zone is
 *  derived from a single VDOT curve (easy = 70% of a RACE VDOT), so a fast
 *  5K makes easy running unrealistically quick and grounds nothing in the
 *  athlete's actual aerobic history, HR, or effort.
 *
 *  This module derives an athlete-SPECIFIC aerobic range for Recovery,
 *  Easy and Long Run from RECENT COMPARABLE easy runs using robust
 *  statistics (median / trimmed percentiles — never the single fastest
 *  run). VDOT is used only as a broad SAFETY boundary. When there isn't
 *  enough aerobic evidence it degrades to a conservative, RPE-first result.
 *
 *  Pure and deterministic. REUSES athleteEngine.classifyWorkout (to pick
 *  valid easy runs) and performance.trainingPaces (VDOT boundary). It does
 *  NOT compute performance paces (marathon/threshold/…) — those stay on the
 *  VDOT curve. The single authoritative pace service composes the two.
 */

import { classifyWorkout } from "./athleteEngine.js";
import { trainingPaces } from "./performance.js";
 ...
 */
export function selectAerobicRuns(activities, fitness, nowMs) {
 ...
 */
export function computeAerobicCalibration(activities, fitness, nowMs) {
  const now = nowMs || Date.now();
  const runs = selectAerobicRuns(activities, fitness, now);
  const vdot = fitness && fitness.vdot != null ? Number(fitness.vdot) : null;
  const vdotEasySec = vdot != null ? trainingPaces(vdot).easy.secPerKm : null;
 ...
 */
export function applyDailyModifiers(range, ctx) {
  ctx = ctx || {};
 ...
 */
export function computeFeedbackBias(feedbackRows) {
  let bias = 0;
...
export const AEROBIC_CALIBRATION_VERSION = "aerobic-calibration-v1";
