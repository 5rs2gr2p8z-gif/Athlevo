...
export function buildRosterEntry(raw) {
  ...
  return {
    ...
    target_event_date: str(p.target_race_date),
    today_planned: today
      ? { title: str(today.title) || str(today.session_type), sport: today.sport ? String(today.sport).toLowerCase() : sport }
      : null,
...
export function buildAthleteOverview(raw) {
  ...
  return {
    ...
    today_planned: raw && raw.todaySession
      ? { title: str(raw.todaySession.title) || str(raw.todaySession.session_type), sport: raw.todaySession.sport || null }
      : null,
