...
export function buildSessionPatch(action, session) {
  ...
  if (changes.duration_minutes !== undefined) {
    patch.duration_minutes = changes.duration_minutes;
  }
  if (changes.distance_km !== undefined) {
    patch.distance_km = changes.distance_km;
  }
  if (changes.session_type !== undefined) {
    patch.session_type = changes.session_type;
  }
  if (changes.title !== undefined) {
    patch.title = changes.title;
  }
  if (changes.intensity !== undefined) {
    patch.intensity = changes.intensity;
  }
