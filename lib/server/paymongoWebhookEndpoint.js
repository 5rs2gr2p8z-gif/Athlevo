...
import { captureServerEventBestEffort, paidActivationProperties } from "./productAnalytics.js";
...
export default async function paymongoWebhookHandler(request, response) {
  ...
  try {
    ...
    if (paid) {
      ...
      if (result && result.applied && result.event_type !== "renewed") {
        captureServerEventBestEffort(
          paid.userId,
          "subscription_activated",
          paidActivationProperties("paymongo_webhook", "paymongo")
        );
      }
