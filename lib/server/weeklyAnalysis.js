...
/*
 * Matches planned (non-rest) sessions to what actually happened.
 *
 * Explicit athlete feedback (workout_execution_records, passed as a Map
 * keyed by session id) is the source of truth: a "completed" or
 * "modified" record counts as completed, a "skipped" record counts as
 * not completed, regardless of Strava. When there is no feedback, the
 * session falls back to same-day activity matching: a session counts as
 * completed when an activity exists on that day (Asia/Manila) and, if
 * the session had a planned duration, the activity covered at least 40%
 * of it.
 */
export function matchPlannedSessions(sessions, activities, executionMap) {
  const feedback =
    executionMap instanceof Map ? executionMap : new Map();
  ...
  const matches = planned.map(session => {
    ...
    const record = session?.id
      ? feedback.get(String(session.id)) || null
      : null;

    const reportedStatus = record?.status || null;

    // Prefer the athlete's own report over inferred matching.
    let completed;
    let actualMinutes;

    if (reportedStatus === "completed" || reportedStatus === "modified") {
      completed = true;
      actualMinutes =
        toNumber(record.actual_duration_minutes) ??
        (best ? activityMinutes : null);
    } else if (reportedStatus === "skipped") {
      completed = false;
      actualMinutes = null;
    } else {
      completed = Boolean(
        best &&
          (!plannedMinutes || activityMinutes >= plannedMinutes * 0.4)
      );
      actualMinutes = best ? activityMinutes : null;
    }
...
export function assessInjuryRisk(checkIn, injuryMemories, executionSignals) {
  // Pain reported on any individual session this week is a first-class
  // signal — it makes pain-related skips available to the next plan.
  if ((executionSignals?.painReports || []).length > 0) {
    return "elevated";
  }
...
export function buildKeyConcerns({
  ...
  const painReport = (executionSignals?.painReports || [])[0];

  if (painReport) {
    const parts = [painReport.location];

    if (painReport.severity) {
      parts.push(`severity ${painReport.severity}/10`);
    }

    concerns.push(
      `Pain reported during a session (${parts
        .filter(Boolean)
        .join(", ")}).`
    );
  } else if (injuryRisk === "elevated") {
...
export function buildNextWeekPriorities({
  ...
  // Schedule-driven skips should reshape the calendar, not cut fitness
  // expectations.
  const scheduleSkips = (executionSignals?.skips || []).filter(
    skip =>
      ["schedule", "travel", "weather"].includes(skip.reason)
  );

  if (
    scheduleSkips.length &&
    injuryRisk !== "elevated" &&
    recovery.status !== "poor"
  ) {
    priorities.push(
      "Fit sessions to the real schedule that caused missed days — keep the training targets intact."
    );
  }
