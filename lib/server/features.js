 ...
 *
 * Paid entitlement requires a verified Whop-owned subscription row. A browser
 * cannot create that row because subscriptions are webhook/service-role only.
 */
...
export function resolveEntitlement(subscription, now = Date.now()) {
  ...
  // Whop's verified webhook is the only authority that may grant paid access.
  if (
    planId === "free" ||
    paidTier === 0 ||
    !whopPaidRow
