...
export function resolveEntitlement(subscription, now = Date.now()) {
  ...
  /*
   * Whop rows historically always mapped to "performance" (Athlevo Pro) —
   * there was only ever one paid Whop product. Athlevo Pro+ introduces a
   * second, genuinely separate paid tier ("elite"): a Whop row explicitly
   * stamped plan_id "elite" (a distinct Pro+ Whop product/webhook mapping)
   * is honoured as elite. Any other/legacy Whop plan_id (including the
   * historical single ₱597 product, unrecognised values, and
   * "founding_beta") still resolves to "performance" so every existing
   * paid subscriber keeps their current Athlevo Pro access unchanged.
   */
