...
import { requireAiConsent, sendAiConsentRequired, anonymousAiConsentGranted } from "./aiConsent.js";
...
function sanitizeBody(body) {
  ...
  return {
    message,
    ai_consent: anonymousAiConsentGranted(body),
  ...
  try {
    ...
    if (token) {
      ...
      if (verified && verified.ok && verified.user && verified.user.id) {
        // Authenticated onboarding diagnostic: authoritative server-read
        // consent gate, same as every other authenticated AI endpoint.
        const consentGate = await requireAiConsent(verified.user.id, "diagnostic");
        if (!consentGate.allowed) return sendAiConsentRequired(res, consentGate);
        ...
        if (!payload.ai_consent) {
          return sendJson(res, 403, {
            error: "AI-powered features are turned off. Enable them to continue.",
    ...
    // fail-open on rate-limit infra only — consent checks above always
    // throw through to their own explicit response, never fail-open.
  }
