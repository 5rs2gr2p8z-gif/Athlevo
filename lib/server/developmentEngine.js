/*
 * ══════════════════════════════════════════════════════════════════════
 *  Athlevo — Development Engine  (Athlete Identity & Progression)
 * ══════════════════════════════════════════════════════════════════════
 *
 *  Makes progress feel like BECOMING a better endurance athlete, not just
 *  completing workouts. Three deterministic, data-driven capabilities:
 *
 *    1. buildProgressions() — the "You are becoming…" development
 *       categories, each with a progress %, a trend (Improving / Plateau /
 *       Declining / Building), a plain-English WHY, and a next milestone.
 *    2. detectAchievements() — meaningful coaching achievements DERIVED
 *       from the athlete's real history (never a hardcoded badge list).
 *    3. buildTimeline() — the athlete's development story as ordered events.
 *
 *  REUSE, don't duplicate: progression % comes from the EXISTING Athlevo
 *  Score components; trend comes from persisted score history; supporting
 *  signals come from the athlete engine + trends. This file adds no new
 *  fitness maths — only the identity/progression narrative around them.
 *
 *  Pure and deterministic. js/developmentEngine.js is a verbatim client
 *  mirror. Future device feeds (Garmin/COROS/WHOOP) only enrich the input
 *  `signals` / `data`; the engine and UI never change.
 */
...
export function buildProgressions(input) {
  input = input || {};
...
export function detectAchievements(data) {
  data = data || {};
 ...
 */
export function snapshotNearest(history, daysAgo, nowMs) {
...
export const DEVELOPMENT_ENGINE_VERSION = "development-engine-v1";
