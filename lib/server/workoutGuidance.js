/*
 * ══════════════════════════════════════════════════════════════════════
 *  Athlevo — Workout Guidance Generator  (Training Engine V2)
 * ══════════════════════════════════════════════════════════════════════
 *
 *  Turns a planned session into athlete-specific, segment-by-segment
 *  execution guidance: warm-up, main set, recovery, cool-down, strides —
 *  each with a target pace RANGE, RPE range, a plain-English instruction,
 *  and concise condition-adjustment rules. Never leaves a bare term like
 *  "Threshold" or "Control" without a target and a meaning.
 *
 *  REUSES the pace service (the one authoritative pace source) so Train
 *  shows exactly what Today shows. Adds NO pace/VDOT maths. Backward
 *  compatible: if the athlete's pace confidence is insufficient it produces
 *  RPE-first guidance, and legacy sessions with no structure still render.
 *  If a session already carries `structured_segments`, those are honoured
 *  and enriched with the athlete's live pace ranges.
 */

import { getZoneGuidance } from "./paceService.js";
 ...
 */
export function generateWorkoutGuidance(session, fitness, options) {
  options = options || {};
...
export const WORKOUT_GUIDANCE_VERSION = "workout-guidance-v1";
