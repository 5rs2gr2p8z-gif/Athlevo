...
/*
 * Canonical version check — the ONE place version comparison happens.
 *
 * Stored records in the wild may carry the version as "recognition-v2",
 * "recognition-v1", a bare number (2), a bare string ("2"), or nothing at all.
 * Normalise both sides to the numeric suffix before comparing, so a mismatched
 * shape is never treated as "already current" and left stale.
 */
...
export function isCurrentRecognitionVersion(recognition) {
  if (!recognition || !recognition.workoutType) return false;
  const cur = versionNumber(RECOGNITION_VERSION);
  const got = versionNumber(recognition.version);
  return got != null && cur != null && got === cur;
}
