/*
 * ══════════════════════════════════════════════════════════════════════
 *  Athlevo — Athlete Engine (Phase B)   ·   authoritative, pure, no I/O
 * ══════════════════════════════════════════════════════════════════════
 *
 *  The single, reusable source of truth for Athlete metrics. Every screen
 *  (Today / Train / Trends / Coach) and the AI coach are meant to read the
 *  SAME numbers from here — nothing is recomputed differently elsewhere.
 *
 *  This module is deterministic and does no I/O. Callers pass in already
 *  loaded rows (activities, execution records, confirmed races, profile)
 *  and get back a complete athlete_metrics snapshot. Persisting it (to the
 *  athlete_metrics table) is the caller's job.
 *
 *  REUSE, don't duplicate:
 *    · VDOT, training paces, VO2max proxy, Current Running Level  →
 *      lib/server/performance.js (imported here — the maths lives once).
 *    · This file ADDS the pieces that did not exist yet: automatic
 *      workout classification, a wearable-free training-load / recovery
 *      model (acute/chronic load, ACWR, monotony, strain, fatigue), a
 *      consistency engine, and the composite Athlevo Score that blends
 *      them. js/athleteEngine.js is a verbatim client mirror (parity
 *      tested) so the browser screens can use the identical logic.
 *
 *  Safety: never invents HR/pace/wearable data. A signal with too little
 *  evidence returns null and is excluded from weighting rather than scored
 *  as zero. Current VDOT only rises on genuinely better race evidence.
 */
...
// "m:ss/km" from seconds-per-km (guards the 5:60 rounding bug).
...
/* ═════════════════════════ 1 · VDOT + paces ═════════════════════════════
 *
 *  Reusable VDOT calculator. Input: distance (m) + time (s). Output: VDOT
 *  and the five headline training paces. Pure reuse of performance.js —
 *  no VDOT maths is re-implemented here.
 */
export function calculateVdotPaces(distanceMeters, durationSeconds) {
  ...
  return {
    ...
    paces: {
      easy: p.easy.pace,
      marathon: p.marathon.pace,
      threshold: p.threshold.pace,
      interval: p.vo2.pace,       // "Interval" == VO2 zone
      repetition: p.repetition.pace
    },
    paceSeconds: {
      easy: p.easy.secPerKm,
      marathon: p.marathon.secPerKm,
      threshold: p.threshold.secPerKm,
      interval: p.vo2.secPerKm,
      repetition: p.repetition.secPerKm
    }
  };
}
 ...
 *
 *  Normalise activities + execution records into one training-item list,
 *  de-duplicated by imported_activity_id (an execution linked to an
 *  imported activity wins — manual/matched feedback over the raw import),
 *  following the product source priority. This is the same dedup rule the
 *  Trends engine uses; it lives here as the reusable definition.
 */
export function mergeTrainingItems(activities, executions) {
  ...
  execs.forEach(e => {
    if (e && e.imported_activity_id != null) consumed.add(String(e.imported_activity_id));
  });
  ...
  // Attach seconds-per-km where computable.
  items.forEach(i => {
    i.paceSec = i.distanceKm > 0 && i.durationMin > 0
      ? (i.durationMin * 60) / i.distanceKm
      : null;
 ...
 *
 *  Automatic type from pace, HR, distance, elapsed time, title and Strava
 *  type. Deterministic priority. `ctx` may carry the athlete's current
 *  pace zones (secPerKm) and long-run threshold so classification is
 *  relative to THIS athlete, not absolute cutoffs.
 */
...
// Perceived-intensity factor per type — used for session load when no RPE
// exists (a wearable-free sRPE equivalent).
...
export function classifyWorkout(item, ctx) {
  ctx = ctx || {};
  ...
  // 1) Title keywords are the strongest single signal.
  ...
  // 2) Terrain: lots of climbing per km → Hill.
  if (elevPerKm != null && elevPerKm >= 18) return done("Hill", 0.65);

  // 3) Pace relative to the athlete's own zones (best signal when known).
  ...
  if (paceSec != null && tZone != null) {
    ...
    // Easy band and slower.
    if (eZone != null && paceSec >= eZone * 1.12 && durMin <= 55) {
      return done("Recovery", 0.6);
    }
    if (durMin >= 90 || km >= longKm) return done("Long Run", 0.75);
    return done("Easy", 0.7);
  }

  // 4) No usable pace zones → heart rate relative to max.
  ...
  // 5) Fallback on duration/distance only.
 ...
 *
 *  Wearable-free load model (Foster session-RPE family). Session load uses
 *  real RPE when present, otherwise duration × the classified intensity
 *  factor. From daily loads we derive acute (7-day) and chronic (28-day)
 *  exponentially-weighted loads, their ratio (ACWR = training balance),
 *  monotony, strain, and 0–100 fatigue/fitness indicators.
 */
export function sessionLoad(item, classification) {
...
function ewma(dailyLoads, tau) {
  // dailyLoads: array of {day:0..N} where 0 = today, ordered oldest→newest.
  // Standard Banister EWMA over per-day totals.
  const alpha = 2 / (tau + 1);
  let value = 0;
...
export function computeTrainingLoad(classifiedItems, nowMs) {
  // Bucket loads into per-day totals for the last 42 days.
  const DAYS = 42;
  const daily = new Array(DAYS).fill(0); // index 0 = 41 days ago … last = today
  ...
  const acute = ewma(daily.slice(DAYS - 7), 7);           // ~7-day EWMA
  const chronic = ewma(daily.slice(DAYS - 28), 28);        // ~28-day EWMA
  ...
  const recoveryTrend =
    prevWeekLoad > 0 ? Math.round(((weeklyLoad - prevWeekLoad) / prevWeekLoad) * 100) : null;

  // 0–100 indicators (relative, deterministic — NOT lab measures).
  const fitnessScore = clamp100((chronic / 6));   // chronic daily ~600 → 100
  const fatigueScore = clamp100((acute / 6));

  // Training balance from ACWR (the classic "sweet spot" ~0.8–1.3).
...
export function computeConsistency(items, profile, nowMs) {
  ...
  // Days missed & consistency use recorded planned sessions when present;
  // schedule/pain skips are NOT counted as non-compliance.
 ...
 *
 *  A composite from multiple systems (NOT just VO2max). Each sub-signal is
 *  0–100 or null; the overall reweights across whatever is available.
 *  Documented weighting (sums to 100 when all present):
 *    Aerobic 20 · Threshold 18 · Speed 10 · Endurance 15 ·
 *    Consistency 15 · Volume 12 · Recovery 5 · Recent performance 5
 *  Fatigue is applied as a small penalty on top (not a positive weight),
 *  so heavy accumulated fatigue tempers the score without erasing fitness.
 */
const SCORE_WEIGHTS = {
  aerobic: 20, threshold: 18, speed: 10, endurance: 15,
...
export function computeComponentScores({ fitness, load, consistency, items, races, nowMs }) {
  ...
  // Aerobic base — engine size + easy volume (needs a couple weeks + runs).
  ...
  // Threshold — needs threshold sessions or a confirmed race.
  ...
  // Recent performance — a confirmed race in the last ~60 days.
...
export function composeAthlevoScore(componentScores, load) {
  ...
  // Minimum-data gate: need ≥3 valid signals incl. aerobic OR recent/threshold.
  const gate = valid >= 3 &&
  ...
  let raw = acc / wsum;
  // Small fatigue penalty when accumulated fatigue is high (ACWR-driven).
 ...
 *
 *  Current VDOT is the BEST demonstrated VDOT from confirmed race /
 *  time-trial / training-effort evidence within ~12 months — so it only
 *  rises on a genuinely better performance ("newer AND an improvement"),
 *  never on a slower run. Confirmed races (race_results) ARE the VDOT
 *  history, so no separate table is needed. Falls back to a flagged
 *  estimate from the best recent hard run when no race exists.
 */
export function resolveCurrentVdot(races, items, nowMs) {
  let best = null;
  ...
  for (const i of items || []) {
    ...
    const v = vdotFromRace(meters, seconds);
    if (v == null) continue;
 ...
 *
 *  The one call every consumer uses. Returns a plain object whose keys map
 *  1:1 to the athlete_metrics table columns, plus a `detail` block for
 *  richer consumers (paces, classifications, component statuses).
 */
export function computeAthleteMetrics({ activities, executions, races, profile, now } = {}) {
  const nowMs = now ? (now instanceof Date ? now.getTime() : now) : Date.now();
  const items = mergeTrainingItems(activities, executions);

  // Current VDOT (best evidence) drives the pace zones the classifier uses.
  const vdotInfo = resolveCurrentVdot(races, items, nowMs);
  const currentVdot = vdotInfo ? vdotInfo.vdot : null;
  const paces = currentVdot != null ? trainingPaces(currentVdot) : null;
  ...
  // Classify every item once; carry the label + load onto it.
  ...
  const fitness = currentRunningLevel({ vdot: currentVdot, estimated: vdotInfo ? vdotInfo.estimated : false });
  ...
  const components = computeComponentScores({
    fitness: fitnessBundle, load, consistency, items, races, nowMs
  });
  ...
  const snapshot = {
    ...
    // Component scores (null when Building — never fabricated zeros)
...
export const ATHLETE_ENGINE_VERSION = "athlete-engine-v1";
