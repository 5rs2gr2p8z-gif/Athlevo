...
/*
 * Whop signs webhooks with your webhook secret. The signature arrives in a
 * header (commonly `x-whop-signature`), sometimes prefixed `sha256=`. We
 * accept both hex and base64 encodings and compare in constant time.
 * rawBody MUST be the exact bytes Whop sent (no re-serialization).
 */
export function verifyWhopSignature(rawBody, signatureHeader, secret) {
  ...
  return timingSafeEqualStr(mac.toString("hex"), provided) ||
         timingSafeEqualStr(mac.toString("base64"), provided);
}
...
export function mapWhopEvent(event, opts = {}) {
  ...
  const isInvalidating = /went_invalid|invalid|expired|cancel|deleted|revoked|failed|past_due/.test(action);
