...
/*
 * Programming access is narrower than read access. The authenticated coach
 * must have the same active assignment used for reads and that assignment must
 * explicitly grant read_write. A missing/legacy permission therefore fails
 * closed and can never be upgraded by a client-supplied flag.
 */
export function canCoachManageAthlete(assignments, coachId, athleteId) {
  const cid = str(coachId);
  const aid = str(athleteId);
  if (!cid || !aid) return false;
  return activeAssignmentsForCoach(assignments, cid).some(
    a => str(a.athlete_id) === aid && str(a.permission_level) === "read_write"
  );
}
