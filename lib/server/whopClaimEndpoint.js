  ...
  try {
    ...
    if (claimed && (reason === "claimed" || reason === "already_claimed")) {
      if (reason === "claimed") {
        ...
        await captureServerEvent(user.id, "whop_pending_purchase_claimed", {
          source: "whop_claim"
        });
        if (result.membership_id) {
          await captureServerEvent(user.id, "subscription_activated", {
            source: "whop_claim"
          });
        }
      }
    ...
    await captureServerEvent(user.id, "whop_pending_purchase_claim_failed", {
      source: "whop_claim",
