...
import { getTrainingPaces } from "./paceService.js";
...
/*
 * V2 (additive): a richer execution detail on top of the core state, adding
 * shortened / extended (magnitude of an easier/harder completion) and
 * race / time_trial (an effort the athlete flagged). Non-breaking — the
 * core classifyWorkoutState is unchanged, so all existing callers/tests are
 * unaffected; new consumers can use the finer grain.
 */
export function classifyExecutionDetail(session, todayKey) {
  ...
  // A recorded race / time-trial (e.g. an unplanned effort logged as
  // modified on this session, or a race-typed session).
...
export function runPlanEngine(input = {}) {
  ...
  // V2 (additive): updated pace targets from the SHARED pace service when the
  // caller supplies fitness, and whether any change wants athlete
  // confirmation. Both are optional and never alter the core outputs.
  let paceTargets = null;
  if (input.fitness && typeof getTrainingPaces === "function") {
    try { paceTargets = getTrainingPaces(input.fitness).zones; } catch (error) { paceTargets = null; }
  }
