...
import { anonymousAiConsentGranted } from "./aiConsent.js";
...
function sanitizeBody(body) {
  ...
  const aiConsent = anonymousAiConsentGranted(body);
  return { message, questionKey, fields, history, aiConsent };
}
...
export default async function coachAnonymousHandler(req, res) {
  ...
  // Anonymous trust boundary: no durable identity exists yet, so a
  // client-sent acknowledgment is the proportionate gate here (see
  // lib/server/aiConsent.js). This is never used for an authenticated
  // request.
