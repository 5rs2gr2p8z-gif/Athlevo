...
function buildSegments(lap, kind) {
  if (lap && lap.reps >= 2) {
    return [
      { kind: "warmup" },
...
function recognizeWorkout(activity, opts) {
  ...
  // 1. Lap structure — the strongest signal.
  lap = detectFromLaps(r.laps, zones);
  if (lap) {
    const zoneName = paceZone(lap.fastestPaceSec, zones);
    type = (zoneName && TYPE_FOR_ZONE[zoneName]) || TYPES.INTERVALS;
    // Very short, very fast reps → Speed; longer sustained reps → Threshold.
    if (lap.avgRepSec <= 90 && (zoneName === "rep" || zoneName === "vo2")) type = TYPES.SPEED;
    confidence = 0.7 + Math.min(0.25, (lap.variation - 1.06) * 1.5);   // more contrast → more sure
    signals.laps = { reps: lap.reps, avgRepSec: Math.round(lap.avgRepSec), variation: Number(lap.variation.toFixed(2)) };
  }
  ...
  return {
    ...
    segments: buildSegments(lap, type),
