...
import { computeAerobicCalibration, applyDailyModifiers } from "./aerobicCalibration.js";
...
// Builds an aerobic zone from a calibrated seconds range + provenance.
function buildAerobicZone(key, rangeSec, aero, mod) {
  ...
  return {
    key,
    label: meta.label,
    paceRange: { minSecPerKm: fast, maxSecPerKm: slow, text: `${formatPace(fast).replace("/km", "")}–${formatPace(slow)}` },
    rpe,
    hr: null,
    explanation: meta.meaning,
    ...
    lastRecalculated: aero.lastRecalculated,
    reason: aero.reason,
    effortFirst: !!(mod && mod.rpeFirst),
    removeIntensity: !!(mod && mod.removeIntensity),
    notes
  };
}
...
export function getTrainingPaces(fitness, options) {
  ...
  // Reuse the existing engine for the centre pace of each zone (VDOT curve).
  const centres = (vdot != null && Number.isFinite(vdot)) ? trainingPaces(vdot) : null;

  // Separate aerobic model (Recovery/Easy/Long) from recent easy runs when
  // activities are supplied — grounds easy pace in real aerobic history
  // rather than a single fast race VDOT.
  const aero = options.activities
    ? computeAerobicCalibration(options.activities, fitness, options.nowMs)
    : null;
  ...
  const zones = ZONE_ORDER.map(key => {
    if (AEROBIC_ZONES.has(key) && aero && aero.valid && aero.ranges[key]) {
      ...
      const mod = applyDailyModifiers(base, daily);
      return buildAerobicZone(key, mod.range, aero, mod);
    }
    // Performance zones (and aerobic fallback when no aerobic history):
    // VDOT-derived, with conservative provenance on the aerobic ones.
    ...
    if (AEROBIC_ZONES.has(key)) {
      ...
      z.source = resolved.source;
      z.confidence = resolved.confidence.code;
      z.reason = resolved.updatedLine;
      z.notes = [];
    }
