/*
 * ══════════════════════════════════════════════════════════════════════
 *  Athlevo — Coach Brain V1  (structured coaching reasoning engine)
 * ══════════════════════════════════════════════════════════════════════
 *
 *  This is NOT a chat. It is a deterministic reasoning layer that explains
 *  WHY Athlevo makes each coaching decision. Given a normalised `signals`
 *  bundle (assembled by the data-collection layer from systems that ALREADY
 *  exist — trends, pace service, Athlevo Score history, readiness, athlete
 *  metrics, plan adaptations), it produces ranked coaching insights, each
 *  with:
 *      observation · reasoning · action · confidence
 *  plus, when the plan changed, "Training Updated" blocks:
 *      previous → new → why.
 *
 *  ARCHITECTURE: data collection → reasoning (THIS FILE) → rendering are
 *  three separate layers. Future device integrations (Garmin/WHOOP/COROS)
 *  only feed richer `signals` in; the reasoning + UI don't change.
 *
 *  Personality: elite endurance coach — professional, scientific, calm.
 *  Never motivational fluff, never robotic, never verbose. Never fabricates:
 *  a rule that lacks sufficient data simply returns nothing.
 *
 *  Pure and deterministic. js/coachBrain.js is a verbatim client mirror.
 */
 ...
 */
export function generateInsights(signals, options) {
  options = options || {};
...
export function explainAdaptation(changes, options) {
  options = options || {};
  const limit = options.limit || 3;
  const out = [];
  (changes || []).forEach(c => {
...
export const COACH_BRAIN_VERSION = "coach-brain-v1";
