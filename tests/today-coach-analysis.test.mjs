...
/* ════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════ */
console.log("\n──── 18. Build outputs stay in parity with source ────");
{
  // dist/ and ios/…/public are generated from index.html by
  // `npm run build:native` / `cap sync`. .af-card-analyze belongs in source;
  // this checks build parity (present in source ⇔ present in the generated
  // output when that output exists), not that the build has never been run.
  let distHtml = "";
  ...
  const sourceHasIt = html.includes(".af-card-analyze");

  if (distHtml) {
    test("dist/index.html is in build parity with source for .af-card-analyze",
      distHtml.includes(".af-card-analyze") === sourceHasIt);
  } else {
    test("dist/index.html not present (will be regenerated by build)", true);
  }
  if (iosHtml) {
    test("ios/…/index.html is in build parity with source for .af-card-analyze",
      iosHtml.includes(".af-card-analyze") === sourceHasIt);
  } else {
